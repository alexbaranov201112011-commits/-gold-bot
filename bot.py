...
CHAT_ID=int(os.environ.get("CHAT_ID") or "6832635346")
async def get_sig():
...
async def start(u,c): await u.message.reply_text("Бот работает 24/7. Команда /signal")
...
def main():
    ...
    app.job_queue.run_repeating(job,interval=3600,first=5)
    app.run_polling()
